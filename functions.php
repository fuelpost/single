...
if ( ! function_exists( 'single_setup' ) ) :
/**
...
function single_setup() {
	...
	add_theme_support( 'custom-background', apply_filters( 'single_custom_background_args', array(
		'default-color' => 'ffffff',
		'default-image' => '',
	) ) );
}
endif;
add_action( 'after_setup_theme', 'single_setup' );
 ...
 */
function single_content_width() {
	$GLOBALS['content_width'] = apply_filters( 'single_content_width', 790 );
}
...
add_action( 'widgets_init', 'single_widgets_init' );

add_action( 'widgets_init', 'single_widgets_init' );
if ( ! function_exists( 'single_fonts_url' ) ) :
/**
 * Register Google fonts for single.
 *
 * Create your own single_fonts_url() function to override in a child theme.
 *
 * @since single 1.0
 *
 * @return string Google fonts URL for the theme.
 */
function single_fonts_url() {
	$fonts_url = '';
	$fonts     = array();
	$subsets   = 'latin,latin-ext';

	/* translators: If there are characters in your language that are not supported by Open Sans, translate this to 'off'. Do not translate into your own language. */
	if ( 'off' !== _x( 'on', 'Open Sans font: on or off', 'single' ) ) {
		$fonts[] = 'Open Sans:400italic,600italic,700italic,300,400,600,700';
	}
...
function single_scripts() {
	...
	wp_enqueue_style( 'single-fonts', single_fonts_url(), array(), null );
